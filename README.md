...
ディレクトリを選んだ場合は即時解析せず、まずディレクトリツリーを表示します。解析したいファイルをツリーから選択したときだけ、同じ Webview 内で左側のツリーを残したまま右側の解析表示を切り替えます。別ファイルを続けて選ぶと、右側の結果だけが更新されます。

デバッグ用の固定入力を素早く試す場合は、`audioWandasAnalyzer.debugFilePath` に既定の音声ファイルまたはディレクトリのパスを設定し、コマンドパレットから `Audio Analyzer: Analyze Debug Path` を実行します。既定値はワークスペース相対の `media/debug` です。このフォルダにはサンプル WAV を入れてあり、直下の対応音声ファイルを順番に解析します。
